...
public class ArchivoPedido {
    ...
    public ArchivoPedido(String nombre, String ruta, VistaPedidosEntregar vistaPedidosEntregar) {
        this.nombre = nombre;
        this.ruta = ruta;
     ...
     *
     * @param Pedido que se va a guardar.
     */
    public void guardar(Pedido Pedido) {
        ...
        try {
            ...
            if (Pedido != null) {
                escribir.write(Pedido.getNumeroPedido() + " - " + Pedido.getFecha() + " - "
                        + Pedido.getCliente().getNombre() + " - " + Pedido.getCliente().getUsuario() + " - "
                        + Pedido.getCliente().getDireccion() + " - " + Pedido.getProducto().getCodigo() + " - "
                        + Pedido.getProducto().getMarca() + " - " + Pedido.getCantidad() + " - "
     ...
     * cola.
     *
     * @param colaPedido donde se guardan los datos leídos.
     */
    public void leer(ColaPedido colaPedido) {
        String linea = ""; // Variable para guardar la línea leída.
        try {
            File directorio = new File(getRuta().replace("\\ColaPedidosPendientes.txt", ""));/*
...
//                Pedido pedido = new Pedido(Integer.parseInt(datos[0]), datos[1], datos[2], datos[3], datos[4], Integer.parseInt(datos[5]), datos[6],
//                        Integer.parseInt(datos[7]),Double.parseDouble( datos[8]),Double.parseDouble(datos[9]));
//                colaPedido.agregar(admin);
